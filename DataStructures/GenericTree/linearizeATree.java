...
public class linearizeATree {
  ...
  public static Node linearizeOP(Node node){
    if(node.children.size()==0) return node;
    Node llin= linearizeOP(node.children.get(node.children.size()-1));
    while(node.children.size()>1){
      Node last=node.children.remove(node.children.size()-1);
      Node slast=node.children.get(node.children.size()-1);
      Node sllin=linearizeOP(slast);
      sllin.children.add(last);
    }
    return llin;
  }
