...
public class Queen1dSafe {

    public static boolean IsQueenSafe(boolean[][] chess, int row, int col) {
        for(int i=row,j=col;i>=0;i--) if(chess[i][j]) return false;
        for(int i=row,j=col;j>=0;j--) if(chess[i][j]) return false;
        for(int i=row,j=col;i>=0&&j>=0;i--,j--) if(chess[i][j]) return false;
        for(int i=row,j=col;i>=0&&j<chess[0].length;i--,j++) if(chess[i][j]) return false;
