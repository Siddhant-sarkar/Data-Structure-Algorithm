...
public class knight1dknightsafe {

    public static boolean IsKnightSafe(boolean[][] chess, int i, int j) {
        if(i-2>=0 && j+1<chess[0].length){
            if(chess[i-2][j+1]) return false;
        }
        if(i-2>=0 && j-1>=0){
            if(chess[i-2][j-1]) return false;
        }

        if(i+2<chess.length && j+1<chess[0].length){
            if(chess[i+2][j+1]) return false;
        }
        ...
        if(j+2<chess[0].length && i+1<chess.length){
            if(chess[i+1][j+2]) return false;
        }
        if(j+2<chess[0].length && i-1>=0){
            if(chess[i-1][j+2]) return false;
        }

        if(j-2>=0 && i+1<chess.length){
            if(chess[i+1][j-2]) return false;
        }
        if(j-2>=0 && i-1>=0){
            if(chess[i-1][j-2]) return false;
